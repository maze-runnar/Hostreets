    ...
    <div class="column">
      <form method="post">
        ...
        <select id="country" name="branch">
          ...
          <option value="canada">MECHANICAL</option>
          <option value="usa">IT</option>
             <option value="usa">CIVIL</option>
             <option value="usa">OTHER(mention your branch in complain section)</option>
        </select>
...
<label>  your feedback/suggestion is related to :</label>
        <select  name="feed">
          ...
          <option value="usa"> E-cell </option>
          <option value="usa">OTHER(mention your branch in complain section)</option>
        </select>
...
$query = "SELECT 'roll' FROM `feedback` where roll='".$roll."'";
        ...
        if($numResults>=1)
        {
            $message = $roll." enrollment number already exist!!";
            
        }
        else
        {
          $quer =
          " INSERT INTO `feedback`(`name`, `roll`, `branch`, `feed`, `subject`) VALUES ($name,$roll,$branch,$feed,$subject)";
